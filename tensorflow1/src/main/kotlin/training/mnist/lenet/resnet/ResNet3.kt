 ...
 *                                                        | ---- identity ------- |
 *     | ---- conv2d(13)  ------ |                       | ---- conv2d(23) ------- |
 *  -- +- conv2d(11)conv2d(12) --+ -> Relu -> MaxPool-> +- conv2d(21)/conv2d(22) --+ -> Relu -> MaxPool -> Flatten -> Dense Top
 */
fun main() {
    ...
    Graph().use { graph ->
        ...
        // Added branch (1 unit conv2d 3x3)
        val truncatedNormal13 = tf.random.truncatedNormal(
            tf.constant(longArrayOf(3, 3, NUM_CHANNELS, 32)),
            Float::class.javaObjectType,
            TruncatedNormal.seed(SEED)
        )

        val conv13Weights: Variable<Float> =
            tf.variable(Shape.make(3L, 3L, NUM_CHANNELS, 32), Float::class.javaObjectType)

        val conv13WeightsInit = tf.assign(conv13Weights, tf.math.mul(truncatedNormal13, tf.constant(0.1f)))

        val conv13 = tf.nn.conv2d(
            images, conv13Weights, mutableListOf(1L, 1L, 1L, 1L),
            PADDING_TYPE
        )

        val conv13Biases: Variable<Float> = tf.variable(Shape.make(32), Float::class.javaObjectType)

        val conv13BiasesInit = tf.assign(
            conv13Biases, tf.zeros(
                constArray(
                    tf,
                    32
                ), Float::class.javaObjectType
        ...
        val relu12 = tf.nn.relu(tf.math.add(tf.nn.biasAdd(conv13, conv13Biases), tf.nn.biasAdd(conv12, conv12Biases)))
        ...
        // Added branch (1 unit conv2d 3x3)
        val truncatedNormal23 = tf.random.truncatedNormal(
            tf.constant(longArrayOf(3, 3, 32, 32)),
            Float::class.javaObjectType,
            TruncatedNormal.seed(SEED)
        )

        val conv23Weights: Variable<Float> =
            tf.variable(Shape.make(3L, 3L, 32, 32), Float::class.javaObjectType)

        val conv23WeightsInit = tf.assign(conv23Weights, tf.math.mul(truncatedNormal23, tf.constant(0.1f)))

        val conv23 = tf.nn.conv2d(
            pool1, conv23Weights, mutableListOf(1L, 1L, 1L, 1L),
            PADDING_TYPE
        )

        val conv23Biases: Variable<Float> = tf.variable(Shape.make(32), Float::class.javaObjectType)

        val conv23BiasesInit = tf.assign(
            conv23Biases, tf.zeros(
                constArray(
                    tf,
                    32
                ), Float::class.javaObjectType
        ...
        // Added identity branch
        val relu22 = tf.nn.relu(
            tf.math.add(
                pool1,
                tf.math.add(
                    tf.nn.biasAdd(conv23, conv23Biases),
                    tf.nn.biasAdd(conv22, conv22Biases)
                )
            )
        )
        ...
        val variablesInit = listOf(
            ...
            conv13WeightsInit, conv13BiasesInit, conv23WeightsInit, conv23BiasesInit,
