...
fun main() {
    ...
    Graph().use { graph ->
        ...
        val labels =
            tf.placeholder(java.lang.Float::class.java)


        val weights: Variable<java.lang.Float> =
            tf.variable(Shape.make(784, 10), java.lang.Float::class.java)
        val weightsInit =
            tf.assign(weights, tf.zeros(constArray(tf, 784, 10), java.lang.Float::class.java))

        val biases: Variable<java.lang.Float> =
            tf.variable(Shape.make(10), java.lang.Float::class.java)
        val biasesInit = tf.assign(biases, tf.zeros(constArray(tf, 10), java.lang.Float::class.java))
        ...
        fun Float.asJava(): java.lang.Float = this as java.lang.Float

//        val LEARNING_RATE: java.lang.Float = //.asJava()

        val gradients = tf.gradients(crossEntropy, Arrays.asList(weights, biases))
//        val alpha = tf.constant(floatArrayOf(0.2f).first())
        val alpha = tf.constant(0.2f) as Constant<java.lang.Float>
        val weightGradientDescent =
            tf.train.applyGradientDescent(weights, alpha, gradients.dy<java.lang.Float>(0))
        val biasGradientDescent =
            tf.train.applyGradientDescent(biases, alpha, gradients.dy<java.lang.Float>(1))
